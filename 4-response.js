...
export function getData() {
  ...
  .then(response => {
    ...
    // return response.text(); // for text or html
    // return response.json(); // for json data
    return response.blob(); //Binary large object for images, fonts, videos.

    resp.text() // for text,html and xml files, and css, and js.
    resp.json() // for json files.
  })
  .then(blob => {
    console.log(blob); // Blob {size: 37603, type: "image/jpeg"} or is a chunk of memory on users computer.
